   ...
   public void spellSelection(Player player) {
     if (ItemLoreStats.plugin.util_WorldGuard != null) {
       if (!ItemLoreStats.plugin.util_WorldGuard.playerInPVPRegion(player)) {
         try {
           if (this.gearStats.getSpellName(player.getInventory().getItemInMainHand()) == null) return;
           ...
           if (hasCooldown(player.getName() + "." + spellName.replaceAll(" ", "").toLowerCase(), spellCooldown)) return;
