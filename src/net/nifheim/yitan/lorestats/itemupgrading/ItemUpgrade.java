...
public class ItemUpgrade {
    ...
    net.nifheim.yitan.lorestats.GearStats gearStats = new net.nifheim.yitan.lorestats.GearStats();
    ...
    public void increaseItemStatOnItemInHand(Player player, ItemStack itemInHand, String handType) {
        if (!Main.getInstance().getConfig().getStringList("disabledInWorlds").contains(player.getWorld().getName())) {
            String damageName = Main.getInstance().getConfig().getString("primaryStats.damage.name");
            String healthName = Main.getInstance().getConfig().getString("primaryStats.health.name");
            String heroesManaName = Main.getInstance().getConfig().getString("heroesOnlyStats.heroesMaxMana.name");
            String levelName = Main.getInstance().getConfig().getString("bonusStats.xpLevel.name");
            ...
            if ((itemInHand != null)
                ...
                if (ChatColor.stripColor(itemLore.toString()).contains(levelName)) {
                    ...
                    if (Main.getInstance().getConfig().getInt("upgradeStatsOnLevelChange.upgradeCap") == 0) {
                        return;
                    }
                    if (upgrades > Main.getInstance().getConfig().getInt("upgradeStatsOnLevelChange.upgradeCap")) {
                        if (Main.getInstance().getConfig().getBoolean("messages.upgradeCapReached")) {
                    ...
                    for (String line : itemLore) {
                        if (globalIndex < itemLore.size()) {
                            ...
                            for (int i = 0; i < Main.getInstance().getConfig().getConfigurationSection("upgradeStatsOnLevelChange.stats").getKeys(false).size(); i++) {
                                String key = Main.getInstance().getConfig().getConfigurationSection("upgradeStatsOnLevelChange.stats").getKeys(false).toString().split(",")[i].trim().replaceAll("&([0-9a-f])", "").replaceAll("\\[", "").replaceAll("\\]", "");
                                String statName = "";

                                if (Main.getInstance().getConfig().getString("primaryStats." + key + ".name") != null) {
                                    statName = Main.getInstance().getConfig().getString("primaryStats." + key + ".name");
                                } else if (Main.getInstance().getConfig().getString("secondaryStats." + key + ".name") != null) {
                                    statName = Main.getInstance().getConfig().getString("secondaryStats." + key + ".name");
                                } else if (Main.getInstance().getConfig().getString("bonusStats." + key + ".name") != null) {
                                    statName = Main.getInstance().getConfig().getString("bonusStats." + key + ".name");
                                } else if (Main.getInstance().getConfig().getString("heroesOnlyStats." + key + ".name") != null) {
                                    statName = Main.getInstance().getConfig().getString("heroesOnlyStats." + key + ".name");
                                }
                                ...
                                if ((lore.replaceAll(this.languageRegex, "").matches(statName)) && (lore.contains("%"))) {
                                    ...
                                    double upgradeNumbers = getNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats." + key);
                                    ...
                                    if (lore.contains("-")) {
                                        ...
                                        double upgradeMinNumbers = getMinNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats.damage");
                                        double upgradeMaxNumbers = getMaxNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats.damage");
                                        ...
                                        String getLine = line;
                                        double getNumbers = Double.parseDouble(ChatColor.stripColor(line).replaceAll("[^0-9.]", ""));
                                        double upgradeNumbers = getNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats.damage");
                ...
                if (Main.getInstance().getConfig().getBoolean("messages.upgradeSuccessful")) {
                            ...
                            for (int i = 0; i < Main.getInstance().getConfig().getConfigurationSection("upgradeStatsOnLevelChange.stats").getKeys(false).size(); i++) {
                                ...
                                if ((lore.replaceAll(this.languageRegex, "").matches(statName)) && (lore.contains("%"))) {
                                    ...
                                    if (lore.contains("-")) {
                                        ...
                                        double upgradeMinNumbers = getMinNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats." + key);
                                        double upgradeMaxNumbers = getMaxNumbers + Main.getInstance().getConfig().getDouble("upgradeStatsOnLevelChange.stats." + key);
                ...
                if (ChatColor.stripColor(itemLore.toString()).contains(levelName)) {
                    ...
                    for (String line : itemLore) {
                        if (globalIndex < itemLore.size()) {
                            ...
                            for (int i = 0; i < Main.getInstance().getConfig().getConfigurationSection("upgradeStatsOnLevelChange.stats").getKeys(false).size(); i++) {
                                String key = Main.getInstance().getConfig().getConfigurationSection("upgradeStatsOnLevelChange.stats").getKeys(false).toString().split(",")[i].trim().replaceAll("&([0-9a-f])", "").replaceAll("\\[", "").replaceAll("\\]", "").trim();
                                String statName = "";
